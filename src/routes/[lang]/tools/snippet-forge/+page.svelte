<script lang="ts">
  ...
  $: jsonLd = {
      ...
      "@type": "SoftwareApplication",
        "@id": $page.url.origin + "/" + lang + "/tools/snippet-forge",
  ...
  const howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to use Snippet Forge",
    "description": "Step-by-step guide to using the Snippet Forge tool.",
    "step": [
        {
            "@type": "HowToStep",
            "name": "Configure Tool",
...
<svelte:head>
  ...
  <link rel="canonical" href={$page.url.origin + "/" + lang + "/tools/snippet-forge"} />
   ...
   <div class="h-[calc(100vh-64px)] min-h-[600px] flex flex-col border-b border-slate-200 dark:border-slate-800 relative z-10">
       ...
       <div class="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 z-20 shrink-0">
           ...
           <div class="flex items-center gap-2">
                ...
                <button class="p-2 text-slate-500 dark:text-slate-400 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors relative min-h-[44px] min-w-[44px]"
                    on:click={() => showHistory = !showHistory}
                    title={dict.history}
                >
                    <History size={20} />
                </button>
                <button class="lg:hidden p-2 text-slate-500 dark:text-slate-400 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors min-h-[44px] min-w-[44px]"
...
<script context="module">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
    <RelatedTools lang={lang as 'en' | 'ko'} currentSlug="snippet-forge" currentCategory="dev" />
  </div>
