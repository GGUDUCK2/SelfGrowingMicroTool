<script lang="ts">
  ...
  $: howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to use Markdown Studio",
    "description": "Step-by-step guide to using the Markdown Studio tool.",
    "step": [
        {
            "@type": "HowToStep",
            "name": "Configure Tool",
      ...
      <div class="relative w-full max-w-sm bg-white dark:bg-slate-900 shadow-2xl h-full overflow-y-auto" transition:slide={{ axis: 'x', duration: 300 }}>
        ...
        <div class="p-4">
           {#if $history}
             ...
             {#if $history.length > 0}
                <button class="w-full mt-4 py-2 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors min-h-[44px] min-w-[44px]"
