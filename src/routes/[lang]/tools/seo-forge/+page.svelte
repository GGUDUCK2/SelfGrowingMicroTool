<script lang="ts">
  ...
  async function saveToHistory() {
    try {
      ...
      triggerToast(dictionary.actions.saved);
    } catch (err) {
  ...
  function restoreHistory(item: SeoHistory) {
     ...
     activeTab = 'meta';
     triggerToast(dictionary.actions.restore);
  }
  ...
  function copyToClipboard(text: string) {
    ...
    triggerToast(dictionary.actions.copied);
  }
  ...
  function downloadHtml() {
      ...
      triggerToast(dictionary.actions.export);
  }
  ...
  async function shareLink() {
      ...
      if (navigator.share) {
          try {
              ...
              triggerToast(dictionary.actions.share);
          }
      } else {
          navigator.clipboard.writeText(shareUrl);
          triggerToast(dictionary.actions.share);
...
<Head
  title={`${tags.title ? `${tags.title} | ` : ''}${dictionary.title}`}
  description={dictionary.description}
...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white pb-20">
  ...
  <div class="bg-indigo-600 text-white py-12 px-4 shadow-lg">
    <div class="max-w-6xl mx-auto">
      <h1 class="text-3xl md:text-4xl font-bold mb-4">{dictionary.title}</h1>
      <p class="text-indigo-100 text-lg max-w-2xl">{dictionary.description}</p>
    </div>
  </div>
             ...
             <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
                 ...
                 <div class="relative group">
                     ...
                     <div class="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-100 dark:border-slate-700 hidden group-hover:block z-10">
                         <button on:click={() => applyTemplate('blog')} class="min-h-[44px] block w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 hover:text-indigo-600">{dictionary.templates.blog}</button>
                         <button on:click={() => applyTemplate('product')} class="min-h-[44px] block w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 hover:text-indigo-600">{dictionary.templates.product}</button>
                         <button on:click={() => applyTemplate('portfolio')} class="min-h-[44px] block w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 hover:text-indigo-600">{dictionary.templates.portfolio}</button>
                         <button on:click={() => applyTemplate('landing')} class="min-h-[44px] block w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 hover:text-indigo-600">{dictionary.templates.landing}</button>
                     </div>
                 </div>
                 ...
                 <div class="flex gap-2">
                     <button
                        ...
                        aria-label={dictionary.actions.magicPaste}
                        type="button"
                            ...
                            <div class="flex items-center gap-2">
                                {#if uniqueProjects.length > 0}
                                    <select
                                        ...
                                        aria-label={dictionary.actions.projectFilter}
                                    >
                                ...
                                {/if}
                                <button on:click={clearHistory} class="text-xs text-red-500 hover:text-red-600 underline min-h-[44px]">{dictionary.actions.clear}</button>
                            </div>
                        </div>
                                 ...
                                 {#each filteredHistory as item (item.id)}
                                     <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors group">
                                         ...
                                         <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                             <button aria-label={dictionary.actions.restore} on:click={() => restoreHistory(item)} class="min-h-[44px] min-w-[44px] flex items-center justify-center p-1.5 hover:bg-white dark:hover:bg-slate-600 rounded text-indigo-600" title={dictionary.actions.restore}>
                                             ...
                                             <button aria-label={dictionary.actions.delete} on:click={() => deleteHistory(item.id)} class="min-h-[44px] min-w-[44px] flex items-center justify-center p-1.5 hover:bg-white dark:hover:bg-slate-600 rounded text-red-500" title={dictionary.actions.delete}>
