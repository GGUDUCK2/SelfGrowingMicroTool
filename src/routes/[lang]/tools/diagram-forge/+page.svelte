<script lang="ts">
  ...
  $: howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to use Diagram Forge",
    "description": "Step-by-step guide to using the Diagram Forge tool.",
    "step": [
        {
            "@type": "HowToStep",
            "name": "Configure Tool",
...
<div class="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-50 dark:bg-slate-900">
    <!-- Sidebar -->
    <div class="fixed inset-y-0 left-0 z-50 h-full bg-white dark:bg-slate-800 shadow-xl transform transition-transform duration-300 md:relative md:translate-x-0 md:shadow-none border-r border-slate-200 dark:border-slate-700 {showSidebar ? 'translate-x-0' : '-translate-x-full'} pt-16 md:pt-0 w-full max-w-sm">
        <div class="absolute top-4 right-4 md:hidden">
            <button class="min-h-[44px] min-w-[44px] p-2 text-slate-500 dark:text-slate-400 min-h-[44px] min-w-[44px]" on:click={() => showSidebar = false}>
                <X class="w-6 h-6" />
            </button>
        </div>
    ...
    {#if showSidebar}
        <button class="min-h-[44px] min-w-[44px] fixed inset-0 bg-black/50 z-40 md:hidden min-h-[44px] min-w-[44px]" on:click={() => showSidebar = false} aria-label="Close Sidebar"></button>
    {/if}
    ...
    <div class="flex-1 flex flex-col min-w-0 h-full">
        ...
        <div class="md:hidden flex items-center p-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
            <button class="min-h-[44px] min-w-[44px] mr-4 text-slate-500 dark:text-slate-400 min-h-[44px] min-w-[44px]" on:click={() => showSidebar = true}>
                <Menu size={24} />
            </button>
...
<div class="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
    <RelatedTools lang={lang as 'en' | 'ko'} currentSlug="diagram-forge" currentCategory="dev" />
  </div>
