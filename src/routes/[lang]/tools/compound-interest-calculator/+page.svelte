<script lang="ts">
  ...
  onMount(async () => {
    try {
      const config = await db.compoundInterestConfig.orderBy('updatedAt').last();
      if (config) {
  ...
  $: {
    errors = {};
    if (principal < 0) errors.principal = lang === 'ko' ? '0 이상의 값을 입력하세요.' : 'Value must be 0 or greater.';
    if (contribution < 0) errors.contribution = lang === 'ko' ? '0 이상의 값을 입력하세요.' : 'Value must be 0 or greater.';
    if (rate < 0) errors.rate = lang === 'ko' ? '0 이상의 값을 입력하세요.' : 'Value must be 0 or greater.';
    if (years <= 0) errors.years = lang === 'ko' ? '0보다 큰 값을 입력하세요.' : 'Value must be greater than 0.';
    if (inflationRate < 0) errors.inflationRate = lang === 'ko' ? '0 이상의 값을 입력하세요.' : 'Value must be 0 or greater.';
  }

  $: results = calculateCompoundInterest(principal, rate, years, contribution, inflationRate, compoundFrequency);
  $: totalInvested = principal + (contribution * 12 * years);
  $: totalInterest = results.length > 0 ? results[results.length - 1].nominalBalance - totalInvested : 0;
  $: realValue = results.length > 0 ? results[results.length - 1].realBalance : 0;
  
  function calculateCompoundInterest(p: number, r: number, y: number, c: number, inflation: number, freq: number) {
  ...
  function downloadCSV() {
    const headers = [
      ...
      lang === 'ko' ? '실질 금액 (인플레이션 반영)' : 'Real Balance (Inflation Adjusted)'
    ];
  ...
  // SVG Chart Helpers
  $: maxBalance = results.length > 0 ? results[results.length - 1].nominalBalance : 100;
</script>
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div class="relative mt-1">
              ...
              <input type="number" bind:value={principal} class="w-full pl-8 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" class:border-red-500={errors.principal} />
            </div>
          </label>
          {#if errors.principal}<p class="text-red-500 text-xs mt-1">{errors.principal}</p>{/if}
        </div>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div class="relative mt-1">
              <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">$</span>
              <input type="number" bind:value={contribution} class="w-full pl-8 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" class:border-red-500={errors.contribution}/>
            </div>
          </label>
          {#if errors.contribution}<p class="text-red-500 text-xs mt-1">{errors.contribution}</p>{/if}
        </div>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div class="relative mt-1">
               <input type="number" step="0.1" bind:value={rate} class="w-full pl-4 pr-8 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" class:border-red-500={errors.rate} />
          ...
          {#if errors.rate}<p class="text-red-500 text-xs mt-1">{errors.rate}</p>{/if}
        </div>

        <div>
           <label class="block text-sm font-medium text-gray-700 mb-1">
             {lang === 'ko' ? '물가 상승률 (인플레이션 %)' : 'Inflation Rate (%)'}
             <div class="relative mt-1">
                <input type="number" step="0.1" bind:value={inflationRate} class="w-full pl-4 pr-8 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" class:border-red-500={errors.inflationRate} />
                <span class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">%</span>
             </div>
           </label>
           {#if errors.inflationRate}<p class="text-red-500 text-xs mt-1">{errors.inflationRate}</p>{/if}
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
              {lang === 'ko' ? '복리 주기' : 'Compound Frequency'}
              <select bind:value={compoundFrequency} class="w-full px-4 py-2 mt-1 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none bg-white">
                  <option value={12}>{lang === 'ko' ? '매월 (12회/년)' : 'Monthly (12/yr)'}</option>
                  <option value={4}>{lang === 'ko' ? '분기별 (4회/년)' : 'Quarterly (4/yr)'}</option>
                  <option value={2}>{lang === 'ko' ? '반기별 (2회/년)' : 'Semiannually (2/yr)'}</option>
                  <option value={1}>{lang === 'ko' ? '매년 (1회/년)' : 'Annually (1/yr)'}</option>
              </select>
            </label>
        </div>
        ...
        <div>
          ...
          {#if errors.years}<p class="text-red-500 text-xs mt-1">{errors.years}</p>{/if}
        </div>
    ...
    <div class="lg:col-span-2 space-y-8">
       ...
       <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
         <div class="bg-indigo-50 rounded-2xl p-5 border border-indigo-100">
           <div class="text-indigo-600 text-xs font-semibold mb-1 uppercase tracking-wide">{lang === 'ko' ? '총 평가액' : 'Future Balance'}</div>
           <div class="text-xl lg:text-2xl font-bold text-gray-900 truncate" title={formatMoney(results.length > 0 ? results[results.length - 1].nominalBalance : 0)}>
            {formatMoney(results.length > 0 ? results[results.length - 1].nominalBalance : 0)}
           </div>
         </div>
         <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <div class="text-purple-600 text-xs font-semibold mb-1 uppercase tracking-wide">{lang === 'ko' ? '실질 가치 (물가반영)' : 'Real Value'}</div>
            <div class="text-lg lg:text-xl font-bold text-gray-900 truncate" title={formatMoney(realValue)}>
             {formatMoney(realValue)}
            </div>
          </div>
         <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
           <div class="text-gray-500 text-xs font-semibold mb-1 uppercase tracking-wide">{lang === 'ko' ? '총 투자 원금' : 'Total Invested'}</div>
           <div class="text-lg font-bold text-gray-900 truncate">{formatMoney(totalInvested)}</div>
         </div>
         <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
           <div class="text-emerald-600 text-xs font-semibold mb-1 uppercase tracking-wide">{lang === 'ko' ? '총 이자 수익' : 'Total Interest'}</div>
           <div class="text-lg font-bold text-emerald-600 truncate">+{formatMoney(totalInterest)}</div>
         </div>
       </div>
       
       <!-- Chart -->
       <div class="bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-gray-100 min-h-[350px] relative">
         ...
         <div class="h-[250px] w-full flex items-end gap-1 relative pl-8 pb-6">
           ...
           {#each results as result, i}
             <!-- Bar Group -->
             <div class="flex-1 flex flex-col justify-end gap-0 relative group h-full">
                ...
                <div class="w-full flex flex-col-reverse items-end h-full">
                     ...
                     <div
                        ...
                        style="height: {((result.nominalBalance - result.realBalance) / maxBalance) * 100}%"
                     ></div>
               ...
               <!-- Tooltip -->
               <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-gray-900 text-white text-xs px-3 py-2 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                 <div class="font-bold mb-1">Year {result.year}</div>
                 <div>Nominal: {formatMoney(result.nominalBalance)}</div>
                 <div class="text-indigo-200">Real: {formatMoney(result.realBalance)}</div>
               </div>
             </div>
           {/each}

           <!-- X-axis labels -->
           <div class="absolute bottom-0 left-8 right-0 flex justify-between text-xs text-gray-400 pt-2">
            <span>Year 1</span>
