<script lang="ts">
  ...
  $: howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to use Id Forge",
    "description": "Step-by-step guide to using the Id Forge tool.",
    "step": [
        {
            "@type": "HowToStep",
            "name": "Configure Tool",
  ...
  <div class="bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 dark:border-slate-800 p-1">
    ...
    <div class="flex p-1 space-x-1 bg-slate-100 dark:bg-slate-800 rounded-2xl mb-6">
        {#each ['generate', 'analyze', 'collision'] as tab}
            <button class="min-h-[44px] min-w-[44px] flex-1 py-3 text-sm font-bold rounded-xl transition-all {activeTab === tab ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'} min-h-[44px] min-w-[44px]" on:click={() => handleTabChange(tab as any)}
            >
                {dict.tabs[tab] || tab}
            </button>
        {/each}
