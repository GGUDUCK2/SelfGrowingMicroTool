...
<div class="relative group p-4 rounded-xl border transition-all duration-300 {getBgColor(slot.dayNight)} {slot.isBusinessHour ? 'ring-1 ring-green-500/50' : ''}">
    ...
    {#if !isHome}
        <button on:click={onRemove}
            class="absolute -top-3 -right-3 h-11 w-11 flex items-center justify-center bg-red-500 text-white rounded-full shadow-lg hover:bg-red-600 transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 z-10 min-h-[44px] min-w-[44px]"
