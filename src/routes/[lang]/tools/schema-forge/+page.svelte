<script lang="ts">
  ...
  $: howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to use Schema Forge",
    "description": "Step-by-step guide to using the Schema Forge tool.",
    "step": [
        {
            "@type": "HowToStep",
            "name": "Configure Tool",
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" transition:fade>
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col border border-slate-200 dark:border-slate-800">
            ...
            <div class="p-4 flex-1 overflow-hidden flex flex-col">
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-2">Paste your <code class="bg-slate-100 dark:bg-slate-800 px-1 rounded">CREATE TABLE</code> statements here.</p>
                <textarea
                            ...
                            <div class="max-h-60 overflow-y-auto space-y-1">
                                {#each projects as p}
                                    <div class="flex items-center group rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                        ...
                                        <button class="p-2 opacity-0 group-hover:opacity-100 text-slate-400 dark:text-slate-500 hover:text-red-500 transition-opacity min-h-[44px] min-w-[44px] flex items-center justify-center"
                     ...
                     <div class="relative">
                        ...
                        {#if showHistory}
                             <div class="absolute top-full right-0 mt-2 w-full max-w-[18rem] bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 p-2 z-50" transition:slide>
                                ...
                                <div class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase px-2 mb-2">{t?.history?.snapshots}</div>
                                    ...
                                    {#if activeProject.snapshots && activeProject.snapshots.length > 0}
                                        {#each activeProject.snapshots as snap}
                                            ...
                                            >
                                                ...
                                                <div class="text-xs text-slate-500 dark:text-slate-400">{new Date(snap.date).toLocaleString()}</div>
                                            </button>
                                        {/each}
                                    {:else}
                                        <div class="px-3 py-2 text-sm text-slate-400 dark:text-slate-500 italic">{t?.history?.empty}</div>
                                    {/if}
                                </div>
                            </div>
                        {/if}
                                ...
                                <div class="space-y-1">
                                    {#each TEMPLATES as tmpl}
                                        ...
                                        >
                                            ...
                                            <div class="text-xs text-slate-500 dark:text-slate-400">{tmpl.description}</div>
                                        </button>
                                    {/each}
                                </div>
                            </div>
