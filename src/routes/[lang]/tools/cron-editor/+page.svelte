<script lang="ts">
  ...
  import {
    parseCronExpression,
    COMMON_PRESETS,
    COMMON_PRESETS_KO} from "$lib/utils/cron";
  ...
  $: howToSchema = {
    ...
    "@type": "HowTo",
    "name": "How to use Cron Editor",
    "description": "Step-by-step guide to using the Cron Editor tool.",
    "step": [
        {
            "@type": "HowToStep",
            "name": "Configure Tool",
      ...
      <div class="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-6">
        ...
        <div class="flex flex-wrap gap-2">
          {#each lang === "ko" ? COMMON_PRESETS_KO : COMMON_PRESETS as preset}
            <button class="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 rounded-full text-sm transition-colors border border-indigo-200 dark:border-indigo-500/20 min-h-[44px] min-w-[44px]"
