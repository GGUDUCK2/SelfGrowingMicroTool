  ...
  <main class="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-grow">
    ...
    <div class="lg:col-span-3 space-y-6">
       ...
       <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4 relative overflow-visible z-20">
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
             <div class="flex items-center bg-gray-100 p-1 rounded-lg w-full sm:w-auto">
                <button
                   class="flex-1 sm:flex-none px-4 py-2 min-h-[44px] text-sm font-medium rounded-md transition-colors {mode === 'analyzer' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}"
                   on:click={() => mode = 'analyzer'}
                >
                   {dict.analyzer}
                </button>
                <button
                   class="flex-1 sm:flex-none px-4 py-2 min-h-[44px] text-sm font-medium rounded-md transition-colors {mode === 'designer' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}"
                   on:click={toggleMode}
                >
                   {dict.designer}
                </button>
          ...
          <div class="flex flex-wrap gap-2 text-xs text-gray-500 font-mono">
             {#each Object.values(dict.operators) as label (label)}
                 <button
                    class="px-3 py-2 min-h-[44px] bg-gray-50 hover:bg-indigo-50 hover:text-indigo-600 rounded border border-gray-200 transition-colors"
             ...
             {/each}
             <!-- Extra Operators -->
             <button class="px-3 py-2 min-h-[44px] bg-gray-50 hover:bg-indigo-50 hover:text-indigo-600 rounded border border-gray-200 transition-colors" aria-label="Insert Implies" on:click={() => expression += ' -> '}>→</button>
             <button class="px-3 py-2 min-h-[44px] bg-gray-50 hover:bg-indigo-50 hover:text-indigo-600 rounded border border-gray-200 transition-colors" aria-label="Insert Equivalent" on:click={() => expression += ' <-> '}>↔</button>
          </div>
          {/if}
       </div>
       ...
       <div class="flex items-center gap-1 border-b border-gray-200 overflow-x-auto overflow-y-hidden pb-1 scrollbar-hide">
          <button
             class="px-4 py-2 min-h-[44px] font-medium text-sm rounded-t-lg transition-colors flex items-center gap-2 border-b-2 whitespace-nowrap {activeTab === 'table' ? 'border-indigo-600 text-indigo-700 bg-indigo-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
          ...
          <button
             class="px-4 py-2 min-h-[44px] font-medium text-sm rounded-t-lg transition-colors flex items-center gap-2 border-b-2 whitespace-nowrap {activeTab === 'circuit' ? 'border-indigo-600 text-indigo-700 bg-indigo-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
          ...
          <button
             class="px-4 py-2 min-h-[44px] font-medium text-sm rounded-t-lg transition-colors flex items-center gap-2 border-b-2 whitespace-nowrap {activeTab === 'kmap' ? 'border-indigo-600 text-indigo-700 bg-indigo-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
          ...
          <button
             class="px-4 py-2 min-h-[44px] font-medium text-sm rounded-t-lg transition-colors flex items-center gap-2 border-b-2 whitespace-nowrap {activeTab === 'simplify' ? 'border-indigo-600 text-indigo-700 bg-indigo-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
