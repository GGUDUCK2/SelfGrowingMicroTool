    ...
    <div class="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex">
      ...
      >
        <div class="flex items-center gap-2">
          <Lock size={16} /> {(dict as any)?.stego?.hide || 'Hide Secret'}
        </div>
      </button>
      <button
      ...
      >
        <div class="flex items-center gap-2">
          <Unlock size={16} /> {(dict as any)?.stego?.reveal || 'Reveal Secret'}
        </div>
      </button>
    </div>
  </div>
  ...
  {#if mode === 'encode'}
    <div in:slide class="space-y-4">
      <div class="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 text-sm text-indigo-800 dark:text-indigo-200">
        <p>{(dict as any)?.stego?.helpEncode || 'Enter a secret message to hide inside this image.'}</p>
      </div>

      <div>
        <label for="secret_message" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{(dict as any)?.stego?.message || 'Secret Message'}</label>
        <textarea
          id="secret_message"
          bind:value={message}
          rows="4"
          ...
          placeholder={(dict as any)?.stego?.placeholder || 'Type your secret here...'}
        ></textarea>
      ...
      >
        ...
        {:else}
          <Lock size={18} /> {(dict as any)?.stego?.encode || 'Encode & Generate Image'}
        {/if}
      </button>
      ...
      {#if resultUrl}
        <div class="mt-6 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 text-center animate-in fade-in slide-in-from-bottom-4">
          ...
          >
            <Download size={18} /> {(dict as any)?.stego?.download || 'Download Image'}
          </button>
        </div>
      {/if}
    </div>
  {:else}
    <div in:slide class="space-y-6">
      <div class="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-xl border border-emerald-100 dark:border-emerald-800 text-sm text-emerald-800 dark:text-emerald-200">
        <p>{(dict as any)?.stego?.helpDecode || 'If this image contains a hidden message generated by File Forge, click below to reveal it.'}</p>
      </div>
      ...
      >
        ...
        {:else}
          <Unlock size={18} /> {(dict as any)?.stego?.decode || 'Decode Message'}
        {/if}
      </button>
      ...
      {#if decodedMessage}
        <div class="mt-6 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-bottom-4">
          <h3 class="text-sm font-bold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
            <FileText size={16} /> {(dict as any)?.stego?.found || 'Hidden Message Found'}
          </h3>
          ...
          >
            {(dict as any)?.stego?.copy || 'Copy to Clipboard'}
          </button>
        </div>
      {/if}
    </div>
