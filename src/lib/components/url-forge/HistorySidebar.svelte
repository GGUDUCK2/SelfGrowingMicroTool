    ...
    <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
        ...
        <div class="flex gap-2">
            ...
            <button
                ...
                disabled={isDeleting}
                class="text-xs font-bold text-red-500 hover:text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 px-3 py-1.5 rounded-lg transition-colors"
                aria-label="Clear all non-starred history"
            >
                Clear
            </button>
            {/if}
            <button on:click={() => dispatch('close')} class="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all" aria-label="Close History">
            ...
            <div transition:slide|local class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm group hover:border-indigo-300 dark:hover:border-indigo-700 transition-all">
                ...
                <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="flex gap-1">
                        <button on:click={() => handleRename(item.id, item.name)} class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors opacity-0 group-hover:opacity-100 text-slate-400 hover:text-indigo-500" aria-label="Rename">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                        <button on:click={() => handleToggleStar(item.id)} class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors" aria-label="Star Item">
                        ...
                        <button on:click={() => handleDelete(item.id)} class="p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-300 hover:text-red-500 dark:text-slate-600 dark:hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100" aria-label="Delete Item">
