<script lang="ts">
  import { Upload, FileImage, ImageIcon } from 'lucide-svelte';
...
<div
  class="relative w-full overflow-hidden rounded-2xl border-2 border-dashed transition-all duration-200 ease-out group cursor-pointer
...
>
  ...
  <div class="flex flex-col items-center justify-center py-16 px-4 text-center">
    ...
    <h3 class="text-xl font-semibold text-slate-200 mb-2">
      {isDragging ? dict.pixelForge.dropZone.title : dict.pixelForge.dropZone.sub}
    </h3>
    <p class="text-slate-400 max-w-sm">
      {dict.pixelForge.dropZone.info}
    </p>
  </div>
</div>
