<script lang="ts">
  ...
  let output = '';
  let errorMsg = '';
  let indentType = 2; // Default to 2 spaces
  let isCopied = false;
  ...
  const format = () => {
    ...
    try {
      ...
      // Remove leading/trailing whitespaces and optional empty lines
      const cleanInput = input.trim().replace(/>\s*</g, '><');

      // Step 2: Use vkbeautify based on selection
      if (indentType === 2) {
          // default is 4 in some versions, but standard vkbeautify might only support string shift
          // So let's provide standard spaces:
          output = vkbeautify.xml(cleanInput, '  ');
