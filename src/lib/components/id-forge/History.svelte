    ...
    <div class="space-y-3">
        {#each $history as item (item.id)}
            <div transition:slide class="group relative flex flex-col md:flex-row md:items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:border-indigo-300 dark:hover:border-indigo-700 transition-all">
                <!-- Main Content -->
                <button class="min-h-[44px] min-w-[44px] flex-1 flex flex-col items-start text-left w-full outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg p-1 -m-1" on:click={() => handleRestore(item)}
                >
                ...
                <div class="flex items-center space-x-1 mt-3 md:mt-0 border-t md:border-t-0 pt-2 md:pt-0 border-slate-100 dark:border-slate-700">
                     <button class="min-h-[44px] min-w-[44px] p-2 text-slate-400 hover:text-yellow-500 transition-colors" on:click|stopPropagation={() => toggleStar(item)}
                    ...
                    <button class="min-h-[44px] min-w-[44px] p-2 text-slate-400 hover:text-red-500 transition-colors" on:click|stopPropagation={() => item.id && deleteItem(item.id)}
