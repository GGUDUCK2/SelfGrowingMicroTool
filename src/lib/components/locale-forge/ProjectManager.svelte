...
<div class="flex flex-wrap sm:flex-nowrap items-center gap-2 mb-4 bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800">
    <div class="relative shrink-0">
        ...
        {#if showList && browser}
            <div class="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 p-2 z-50 whitespace-normal">
                <div class="flex justify-between items-center mb-2 px-2">
                    <span class="text-xs font-bold text-slate-500 uppercase">Projects</span>
                    <button class="p-1 min-h-[44px] min-w-[44px] hover:bg-slate-100 dark:hover:bg-slate-700 rounded flex items-center justify-center" on:click={newProject}>
                        <Plus size={14} />
                    </button>
                </div>
    ...
    <input
        ...
        class="bg-transparent border-none focus:ring-0 min-h-[44px] shrink-0 text-sm font-medium text-slate-600 dark:text-slate-300 w-48"
    ...
    <button
        class="flex items-center gap-2 px-3 py-2 min-h-[44px] min-w-[44px] shrink-0 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-md transition-colors"
