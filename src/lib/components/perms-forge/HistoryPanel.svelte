...
<div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 h-full flex flex-col">
    <div class="flex justify-between items-center mb-4">
        ...
        <button on:click={() => clearHistory('perms-forge')} class="min-h-[44px] min-w-[44px] text-xs text-red-500 hover:text-red-600 font-medium px-2 py-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
            Clear
        </button>
    </div>
        ...
        {#if $history$ && $history$.length > 0}
            {#each $history$ as item}
                <div class="group flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all shadow-sm hover:shadow">
                    <button class="flex-1 text-left min-h-[44px] min-w-[44px]" on:click={() => restore(item)}>
                    ...
                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button on:click={() => toggleStar(item.id)} class="p-1.5 rounded hover:bg-slate-100 dark:hover:bg-slate-700 {item.starred ? 'text-yellow-400' : 'text-slate-400'} min-h-[44px] min-w-[44px]" aria-label="Toggle Star">
                        ...
                        <button on:click={() => deleteHistoryItem(item.id)} class="p-1.5 rounded hover:bg-red-100 dark:hover:bg-red-900/30 text-slate-400 hover:text-red-500 min-h-[44px] min-w-[44px]" aria-label="Delete">
