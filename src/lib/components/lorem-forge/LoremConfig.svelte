  ...
  <div class="grid grid-cols-1 gap-6">
    ...
    <div class="space-y-2">
      <label for="dictionary-select" class="block text-sm cursor-pointer font-medium text-slate-700 dark:text-slate-300">
        {dict?.config?.dictionary || 'Dictionary'}
      </label>
      <select
      ...
      >
        <option value="standard">{dict?.config?.dictionaries?.standard || 'Standard Latin'}</option>
        <option value="corporate">{dict?.config?.dictionaries?.corporate || 'Corporate Jargon'}</option>
        <option value="tech">{dict?.config?.dictionaries?.tech || 'Tech Buzzwords'}</option>
        <option value="hangul">{dict?.config?.dictionaries?.hangul || 'Hangul (Korean)'}</option>
      </select>
    </div>
    ...
    <div class="space-y-2">
      <label for="format-select" class="block text-sm cursor-pointer font-medium text-slate-700 dark:text-slate-300">
        {dict?.config?.format || 'Format'}
      </label>
      <select
        id="format-select" bind:value={format}
        class="w-full min-h-[44px] px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-slate-900 dark:text-white transition-all"
      >
        <option value="plain">{dict?.config?.formats?.plain || 'Plain Text'}</option>
        <option value="html">{dict?.config?.formats?.html || 'HTML Tags'}</option>
        <option value="markdown">{dict?.config?.formats?.markdown || 'Markdown'}</option>
      </select>
    </div>
    ...
    <div class="space-y-2">
      <span class="block text-sm cursor-pointer font-medium text-slate-700 dark:text-slate-300">
        {dict?.config?.targetType || 'Target By'}
      </span>
      <div class="flex p-1 bg-slate-100 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
        ...
        >
          ...
          <span class="hidden sm:inline">{dict?.config?.targetTypes?.paragraphs || 'Paragraphs'}</span>
        </button>
        <button
        ...
        >
          <Type class="w-4 h-4" />
          <span class="hidden sm:inline">{dict?.config?.targetTypes?.words || 'Words'}</span>
        </button>
        <button
        ...
        >
          <Hash class="w-4 h-4" />
          <span class="hidden sm:inline">{dict?.config?.targetTypes?.characters || 'Characters'}</span>
        </button>
        <button
        ...
        >
          ...
          <span class="hidden sm:inline">{dict?.config?.targetTypes?.bytes || 'Bytes'}</span>
        </button>
      </div>
    </div>
  ...
  {#if dictionary === 'standard'}
    <div class="flex items-center space-x-3 pt-4 border-t border-slate-100 dark:border-slate-700">
      ...
      <span class="text-sm font-medium text-slate-700 dark:text-slate-300">
        {dict?.config?.startWithLorem || "Start with 'Lorem ipsum...'"}
      </span>
    </div>
  {/if}
