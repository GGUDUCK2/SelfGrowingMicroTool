...
<div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col h-full">
    <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
        ...
        <button on:click={clearAll} class="text-xs text-red-500 hover:text-red-700 font-medium min-h-[44px] min-w-[44px]">
            {dict.clear}
        </button>
    </div>
        ...
        {#if $history && $history.length > 0}
            <div class="divide-y divide-gray-100">
                {#each $history as item}
                    <div class="p-3 hover:bg-slate-50 dark:bg-slate-800 transition-colors flex items-center justify-between group">
                        <button on:click={() => restore(item)} class="text-left flex-1 min-w-0 min-h-[44px]">
                            <div class="font-mono text-sm text-slate-800 dark:text-slate-200 truncate">{item.expression}</div>
                            <div class="text-xs text-slate-400 dark:text-slate-500 mt-1">{formatDate(item.createdAt)}</div>
                        </button>
                        <button on:click={() => item.id && deleteItem(item.id)} class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity min-h-[44px] min-w-[44px] flex items-center justify-center" aria-label={dict.delete}>
