...
<div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col h-full">
  <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
    ...
    {#if data}
      <span class="text-xs font-mono bg-gray-200 text-slate-600 dark:text-slate-400 px-2 py-1 rounded-md">
        ...
        <thead class="bg-slate-50 dark:bg-slate-800 sticky top-0 z-10">
          <tr>
            {#each data.variables as v (v)}
              <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider font-mono">{v}</th>
            {/each}
          ...
          {#each data.rows as row, i (i)}
            <tr class="hover:bg-indigo-50/30 transition-colors">
              {#each row.values as val, j (j)}
                <td class="px-6 py-4 whitespace-nowrap text-sm font-mono {val ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-400 dark:text-slate-500'}">
                  {val ? '1' : '0'}
                </td>
              {/each}
              <td
                class="px-6 py-4 min-h-[44px] min-w-[44px] whitespace-nowrap text-right text-sm {editable ? 'cursor-pointer hover:bg-slate-100 dark:bg-slate-800 ring-inset focus:ring-2 focus:ring-indigo-500 rounded-md outline-none' : ''}"
              ...
              >
                ...
                {#if editable}
                  <span class="ml-2 text-xs text-slate-400 dark:text-slate-500 opacity-0 group-hover:opacity-100">{dict.clickToToggle}</span>
                {/if}
              </td>
            </tr>
          {/each}
  ...
  {:else}
    <div class="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 p-8">
