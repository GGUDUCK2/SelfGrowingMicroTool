...
<div class="flex flex-col h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 w-64 shrink-0">
    <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
        ...
        <button
            class="p-1.5 hover:bg-indigo-50 text-indigo-600 dark:hover:bg-indigo-900/30 dark:text-indigo-400 rounded-lg transition-colors"
            on:click={addTable}
    ...
    <div class="flex-1 overflow-y-auto p-2 space-y-1">
        {#each schema.tables as table (table.id)}
            ...
            >
                ...
                <button
                    class="opacity-0 group-hover:opacity-100 p-1 hover:bg-white dark:hover:bg-slate-700 hover:text-red-500 rounded transition-all z-10"
