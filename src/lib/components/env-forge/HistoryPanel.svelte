...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
  <div class="flex justify-between items-center mb-6">
      ...
      <button class="min-h-[44px] min-w-[44px] px-3 py-1 flex items-center justify-center text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition" on:click={clearHistory}>
          {t.clear}
      </button>
  </div>
          ...
          {#each $historyItems as item}
              <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700 transition">
                  ...
                  <div class="flex items-center space-x-1 shrink-0">
                      <button class="min-h-[44px] min-w-[44px] p-2 flex items-center justify-center text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition" on:click={() => item.id && toggleStar(item.id)} aria-label="Star">
                      ...
                      <button class="min-h-[44px] min-w-[44px] p-2 flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition" on:click={() => item.id && deleteItem(item.id)} aria-label="Delete">
