          ...
          <div class="flex items-center gap-2 relative">
              <div class="relative">
                  ...
                  <div class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 transition-all z-50 overflow-hidden">
                      <button class="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition focus:bg-slate-100 dark:focus:bg-slate-700" on:click={() => { content = "PORT=3000\nDATABASE_URL=postgres://user:pass@localhost:5432/db\nAPI_KEY=sk_test_12345\nDEBUG=true"; isTemplatesOpen = false; handleContentChange(new CustomEvent('change', {detail: content})); }}>
                          Generic
                      </button>
                      <button class="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition focus:bg-slate-100 dark:focus:bg-slate-700" on:click={() => { content = "# Next.js Example\nNEXT_PUBLIC_API_URL=http://localhost:3000/api\nNEXTAUTH_URL=http://localhost:3000\nNEXTAUTH_SECRET=generate_a_random_secret_here\nDATABASE_URL=postgres://user:pass@localhost:5432/nextjs_db"; isTemplatesOpen = false; handleContentChange(new CustomEvent('change', {detail: content})); }}>
                          Next.js
                      </button>
                      <button class="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition focus:bg-slate-100 dark:focus:bg-slate-700" on:click={() => { content = "# SvelteKit Example\nPUBLIC_SUPABASE_URL=https://your-project.supabase.co\nPUBLIC_SUPABASE_ANON_KEY=your_anon_key\nSUPABASE_SERVICE_ROLE_KEY=your_service_role_key"; isTemplatesOpen = false; handleContentChange(new CustomEvent('change', {detail: content})); }}>
                          SvelteKit + Supabase
                      </button>
                  </div>
                  {/if}
