<script lang="ts">
  ...
  $: variantClasses =
    variant === 'primary'
      ? `bg-indigo-500 hover:bg-indigo-600 text-white disabled:opacity-50 disabled:cursor-not-allowed relative border border-transparent`
      : variant === 'danger'
      ? `bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 dark:text-red-400`
  ...
  $: classes = `${baseClasses} ${variantClasses} ${className}`;
