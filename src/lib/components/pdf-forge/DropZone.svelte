...
<div
  role="button"
  tabindex="0"
  class="relative border-2 border-dashed rounded-2xl p-12 text-center transition-all duration-200 cursor-pointer overflow-hidden group {isDragging ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-300 dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-slate-50 dark:hover:bg-slate-800/50'}"
  on:dragover={handleDragOver}
  on:dragleave={handleDragLeave}
  on:drop={handleDrop}
  on:click={() => document.getElementById('pdf-upload')?.click()}
  on:keydown={(e) => { if(e.key === 'Enter' || e.key === ' ') document.getElementById('pdf-upload')?.click(); }}
>
  <input
    type="file"
    id="pdf-upload"
