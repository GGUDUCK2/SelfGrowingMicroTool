...
{#if isOpen}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" transition:fade on:click={() => dispatch('close')}>
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-md w-full p-6 relative" transition:scale on:click|stopPropagation>
      <button class="min-h-[44px] min-w-[44px] absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200" on:click={() => dispatch('close')}>
        <X class="w-5 h-5" />
      </button>
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-2 justify-end pt-2">
            <button class="min-h-[44px] min-w-[44px] px-4 py-2 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 font-medium" on:click={clear}>
                {dict.actions?.clear || "Clear"}
            </button>
            <button class="min-h-[44px] min-w-[44px] px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold transition-colors shadow-lg shadow-indigo-500/20" on:click={save}>
