...
<div class="space-y-6 p-4 bg-slate-800 rounded-xl border border-slate-700 shadow-sm">
  ...
  <div>
    ...
    <div class="grid grid-cols-3 sm:grid-cols-4 gap-2">
      {#each types as type}
        <button
          class="px-3 py-2 text-sm rounded-lg border transition-all duration-200 {state.type === type ? 'bg-indigo-600 border-indigo-500 text-white font-medium shadow-md ring-2 ring-indigo-500/20' : 'bg-slate-700 border-slate-600 text-slate-300 hover:bg-slate-600 hover:border-slate-500'} min-h-[44px] min-w-[44px]"
             ...
             <div class="flex items-center gap-3">
                {#if state.design.logo}
                    <div class="relative w-12 h-12 bg-white rounded-lg p-1 border border-slate-600">
                        ...
                        <button on:click={clearLogo} class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-0.5 shadow-md hover:bg-red-600 min-h-[44px] min-w-[44px]">
                            <X size={12} />
                        </button>
                    </div>
                {/if}
