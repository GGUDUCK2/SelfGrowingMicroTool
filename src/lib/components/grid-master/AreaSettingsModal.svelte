<script lang="ts">
  ...
  const contentTypes = [
      { val: 'none', icon: Type },
      { val: 'hero', icon: LayoutDashboard },
      { val: 'chart', icon: BarChart3 },
      { val: 'table', icon: Table },
      { val: 'form', icon: FormInput },
      { val: 'login', icon: LogIn },
      { val: 'video', icon: Video },
      { val: 'image', icon: ImageIcon },
      { val: 'pricing', icon: DollarSign },
      { val: 'team', icon: Users },
      { val: 'testimonial', icon: Quote },
      { val: 'map', icon: MapPin }
  ];
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden" transition:scale={{ duration: 200, start: 0.95 }}>
          ...
          <div class="p-6 space-y-4">
              ...
              <div>
                  ...
                  <div class="grid grid-cols-3 gap-2 max-h-[160px] overflow-y-auto p-1 custom-scrollbar">
                      {#each contentTypes as t (t.val)}
                          <button
                            class="flex flex-col items-center justify-center p-2 rounded border transition-all text-center gap-1
                            {contentType === t.val
                          ...
                          >
                              ...
                              <span class="text-[10px] font-medium leading-tight">{dict.contentTypes?.[t.val] || t.val}</span>
                          </button>
                      {/each}
                  </div>
              </div>
