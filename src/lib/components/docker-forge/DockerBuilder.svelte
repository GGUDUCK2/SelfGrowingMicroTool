...
`;
  ...
  function estimateLayerSize(line: string): string | null {
    const l = line.trim();
    if (l.startsWith('FROM ')) {
      const img = l.substring(5).split(' ')[0];
      return imageSizeMap[img] || '~100MB';
    }
    if (l.startsWith('RUN ')) {
      const lower = l.toLowerCase();
      if (lower.includes('npm ci') || lower.includes('npm install')) return '~50-200MB';
      if (lower.includes('pip install')) return '~20-100MB';
      if (lower.includes('apt-get') || lower.includes('apk add')) return '~10-50MB';
      if (lower.includes('go build') || lower.includes('cargo build')) return '~10-30MB';
    }
    if (l.startsWith('COPY ')) return '< 1MB (Source)';
    return null;
  }

  $: dockerfileLinesWithSizes = dockerfile.split('\n').map(line => {
    return { line, size: estimateLayerSize(line) };
  });
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4">
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <Button variant="primary" class="text-sm min-h-[44px] min-w-[44px]" aria-label={d.parseAction || "Parse Dockerfile"} on:click={handleSmartPaste}>
  ...
  <div class="space-y-6">
      ...
      <div class="p-0 overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <tbody class="text-slate-300 font-mono text-sm">
            {#each dockerfileLinesWithSizes as { line, size }}
              <tr class="hover:bg-slate-800/50 transition-colors group">
                <td class="py-1 px-4 whitespace-pre">
                  <code class="language-docker"><!-- eslint-disable-next-line svelte/no-at-html-tags -->{@html Prism.highlight(line, Prism.languages.docker, 'docker')}</code>
                </td>
    ...
    <div class="bg-slate-900 rounded-xl shadow-lg overflow-hidden border border-slate-700">
      ...
      <div class="p-4 overflow-x-auto">
        <pre class="text-slate-300 font-mono text-sm leading-relaxed whitespace-pre-wrap select-all"><code class="language-yaml"><!-- eslint-disable svelte/no-at-html-tags -->{@html Prism.highlight(githubActions, Prism.languages.yaml, 'yaml')}</code></pre>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2">
      <div class="grid grid-cols-2 gap-2">
        <Button variant="primary" class="w-full min-h-[44px] min-w-[44px] flex items-center justify-center gap-2" aria-label={d.save} on:click={saveToHistory}>
          <Save size={18} /> {d.save}
        </Button>
        <Button variant="secondary" class="w-full min-h-[44px] min-w-[44px] flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white border-none" aria-label={d.downloadZip || 'Download ZIP'} on:click={downloadZip}>
