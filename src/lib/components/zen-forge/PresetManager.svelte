<script lang="ts">
    ...
    let mixes = liveQuery(() => db.zenForgeMixes.toArray());
...
<div class="flex flex-col gap-4 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
    <div class="flex items-center justify-between">
        ...
        <button on:click={() => showSave = !showSave} class="flex items-center gap-2 px-3 py-1 bg-slate-700 hover:bg-indigo-600 rounded text-xs font-medium transition-colors text-white">
            <Save size={14} />
            {dict.controls.save}
        </button>
    </div>
    ...
    {#if showSave}
        <div class="flex gap-2 animate-in slide-in-from-top-2">
            <input type="text" bind:value={mixName} placeholder={dict.controls.mixName} class="bg-slate-900 border border-slate-600 rounded px-3 py-2 text-sm flex-1 text-white focus:border-indigo-500 outline-none" />
            <button on:click={save} class="bg-indigo-600 px-4 rounded text-sm font-medium text-white hover:bg-indigo-500">Save</button>
        </div>
    {/if}
        ...
        {#if $mixes && $mixes.length > 0}
            {#each $mixes as mix (mix.id)}
                <div class="flex items-center justify-between p-2 bg-slate-700/30 rounded hover:bg-slate-700/80 hover:border-l-4 hover:border-l-indigo-500 transition-all group cursor-pointer border border-transparent"
                    ...
                    <button on:click|stopPropagation={() => remove(mix.id!)} class="text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1">
                        <Trash2 size={14} />
                    </button>
                </div>
            {/each}
