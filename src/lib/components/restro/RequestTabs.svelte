...
<div class="flex flex-col h-full bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
  <div class="flex border-b border-slate-200 dark:border-slate-700">
    <button
      class="px-4 py-2 text-sm font-medium border-b-2 transition-colors {activeTab === 'params' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}"
    ...
    <button
      class="px-4 py-2 text-sm font-medium border-b-2 transition-colors {activeTab === 'headers' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}"
    ...
    <button
      class="px-4 py-2 text-sm font-medium border-b-2 transition-colors {activeTab === 'body' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}"
      ...
      <div class="space-y-2">
        ...
        {#each params as param, i}
          <div class="flex gap-2 items-center group">
            <input type="checkbox" bind:checked={param.enabled} class="rounded text-indigo-600 focus:ring-indigo-500 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
            <input
              type="text"
              bind:value={param.key}
              placeholder="Key"
              class="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-2 py-1 text-sm font-mono text-slate-700 dark:text-slate-200 focus:ring-1 focus:ring-indigo-500 outline-none"
            />
            <input
              type="text"
        ...
        <button
          on:click={addParam}
          class="flex items-center gap-1 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:underline mt-2"
        >
    ...
    {:else if activeTab === 'headers'}
      <div class="space-y-2">
        ...
        {#each headers as header, i}
          <div class="flex gap-2 items-center group">
            <input type="checkbox" bind:checked={header.enabled} class="rounded text-indigo-600 focus:ring-indigo-500 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
            <input
              type="text"
      ...
      <div class="flex flex-col h-full">
        <div class="flex gap-4 mb-3 text-sm">
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="radio" bind:group={bodyType} value="none" class="text-indigo-600 focus:ring-indigo-500" />
            <span class="text-slate-700 dark:text-slate-300">None</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="radio" bind:group={bodyType} value="json" class="text-indigo-600 focus:ring-indigo-500" />
            <span class="text-slate-700 dark:text-slate-300">JSON</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="radio" bind:group={bodyType} value="text" class="text-indigo-600 focus:ring-indigo-500" />
