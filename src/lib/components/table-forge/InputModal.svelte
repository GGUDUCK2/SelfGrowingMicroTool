  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" on:click|self={() => open = false}>
      <div class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[80vh]">
          <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              ...
              <button on:click={() => open = false} class="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded">
                  <X size={20} />
              </button>
          </div>
          ...
          <div class="p-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-2">
              <button on:click={() => open = false} class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded">Cancel</button>
              <button on:click={handleImport} class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded shadow-sm">Import</button>
          </div>
      </div>
  </div>
{/if}
