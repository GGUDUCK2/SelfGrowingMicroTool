...
<div class="h-full flex flex-col bg-slate-50 dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800">
  <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
      ...
      <button on:click={clearHistory} class="p-1 text-slate-400 hover:text-red-500 transition-colors" title={dict.clear}>
          <Trash2 size={16} />
      </button>
  </div>
  ...
  <div class="flex-1 overflow-y-auto p-2 space-y-2">
      {#if $history && $history.length > 0}
          {#each $history as item}
              ...
              <div class="group relative bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-500 transition-all cursor-pointer"
                  ...
                  <button class="absolute top-2 right-2 p-1 text-slate-400 hover:text-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity"
                          on:click|stopPropagation={() => copy(item.result)} title="Copy Result">
                      <Copy size={14} />
                  </button>
              </div>
          {/each}
