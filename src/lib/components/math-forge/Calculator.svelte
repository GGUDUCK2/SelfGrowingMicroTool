...
<div class="max-w-md mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
  <div class="p-6 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
    ...
    <input
      type="text"
      bind:value={input}
      class="w-full bg-transparent text-right text-3xl font-bold text-slate-900 dark:text-white outline-none font-mono"
      placeholder="0"
  ...
  <div class="p-4 grid grid-cols-4 gap-2">
    {#each buttons as row}
      {#each row as btn}
        <button
          class="h-12 rounded-lg font-medium text-lg transition-colors
