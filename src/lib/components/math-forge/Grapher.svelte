...
<div class="flex flex-col gap-4">
  ...
  <div class="space-y-2">
      {#each functions as fn, i}
          <div class="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700 min-h-[44px] min-w-[44px]">
              ...
              <input type="text" aria-label="Function Expression {i+1}" bind:value={fn.expression} on:input={draw} class="flex-1 min-h-[44px] min-w-[44px] bg-transparent border-none outline-none font-mono text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400" placeholder="e.g. sin(x)" />
