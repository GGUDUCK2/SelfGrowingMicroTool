    ...
    {#if parts[activeTab].includes('/')}
       <div class="mt-4 p-4 bg-white/5 rounded-lg">
           <div class="flex items-center space-x-4">
               ...
               <input type="number" min="0" max="60" class="bg-slate-700 text-slate-50 border border-slate-600 rounded-lg px-2 py-1 w-20 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 outline-none" aria-label="{lang === 'ko' ? '시작 값' : 'Start value'}" bind:value={stepState.start} on:change={() => updateStep(activeTab)} />
               <span class="text-gray-300">/</span>
               <span class="text-gray-300">{lang === 'ko' ? '간격' : 'Step'}:</span>
               <input type="number" min="1" max="60" class="bg-slate-700 text-slate-50 border border-slate-600 rounded-lg px-2 py-1 w-20 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 outline-none" aria-label="{lang === 'ko' ? '간격 값' : 'Step value'}" bind:value={stepState.step} on:change={() => updateStep(activeTab)} />
           </div>
