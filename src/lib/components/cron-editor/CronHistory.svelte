...
<div class="bg-white/5 border border-white/10 rounded-xl p-6 min-h-[200px]">
  ...
  {:else}
    <div
      class="space-y-2 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent"
    >
      {#each $history as item (item.id)}
        <div
          class="group flex items-center justify-between p-3 rounded-lg bg-black/20 hover:bg-black/40 border border-white/5 hover:border-white/10 transition-all"
          transition:slide
        >
          <button
          ...
          >
            <div class="flex items-center space-x-2">
              <span class="font-mono text-indigo-300 font-medium"
            ...
            <p
              class="text-xs text-gray-400 mt-1 truncate max-w-[200px] lg:max-w-[300px]"
            >
              {item.description}
            </p>
          </button>
