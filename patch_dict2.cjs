...
// We need to inject additional properties into the smooth and neumorphism objects in BOTH dicts.
// We'll replace the existing properties to add the new ones.
...
dictContent = dictContent.replace(
    /remove: "Remove",\s*reverseOrder: "Reverse Order"/g,
    \`remove: "Remove",
...
// KO Patch (the second occurrence of these replacements)
// Actually the previous regex matched the KO block too if I use global, but let's do it carefully.
dictContent = dictContent.replace(
...
// We need layerPrefix and alpha for KO
dictContent = dictContent.replace(
